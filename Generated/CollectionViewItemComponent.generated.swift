...
class ApplicationKeyboardBindingItemViewController: NSViewController, Component {
  private let layout: NSCollectionViewFlowLayout
  private let dataSource: ApplicationKeyboardBindingItemDataSource
  lazy var scrollView = NSScrollView()
  let collectionView: NSCollectionView
  ...
  init(title: String? = nil,
       ...
       collectionView: NSCollectionView? = nil) {
    self.layout = layout
    self.dataSource = ApplicationKeyboardBindingItemDataSource(title: title, iconController: iconController)
    if let collectionView = collectionView {
      self.collectionView = collectionView
    } else {
      self.collectionView = NSCollectionView()
  ...
  func model(at indexPath: IndexPath) -> ApplicationKeyboardBindingItemModel {
    return dataSource.model(at: indexPath)
  }
...
class ApplicationKeyboardBindingItemDataSource: NSObject, NSCollectionViewDataSource {

  private var title: String?
  private var models = [ApplicationKeyboardBindingItemModel]()
  private(set) var iconController: IconController

  init(title: String? = nil,
       models: [ApplicationKeyboardBindingItemModel] = [],
       iconController: IconController) {
    self.title = title
    self.models = models
    self.iconController = iconController
  ...
  func reload(_ collectionView: NSCollectionView,
              with models: [ApplicationKeyboardBindingItemModel],
              then handler: (() -> Void)? = nil) {
    let old = self.models
    let new = models
    let changes = DiffManager().diff(old, new)
...
struct ApplicationKeyboardBindingItemModel: Hashable {
